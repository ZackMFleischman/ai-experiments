...
Sibling app projects have their own entry points — **`hive/`** (a two-player Hive PWA, built and live), **`lex/`** (a two-player crossword-tile-game PWA, built), **`sudoku/`** (a zero-backend solo sudoku PWA, first title of the minimalist-apps brand — strategy in `MINIMALIST-APPS-STRATEGY.md`, phased status in `BRAND-IMPLEMENTATION.md`), and **`stillness/`** (a zero-backend meditation-timer PWA, the brand's first utility title) — plus **`parlor/`**, the shared platform workspace (core/web/server/harness for two-player games; solo/brand for zero-backend solo apps; native for their $1 Capacitor store wraps), and **`katmai-bears/`** (a React PWA dashboard of the Katmai / Brooks Falls live bear cams). See each directory's `CLAUDE.md`; everything below this line is loom-specific and does not apply there.
