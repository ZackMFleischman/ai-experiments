...
// @parlor/harness, with the tafl machine checks — every board renders all
// 121 squares and fits the viewport.
...
test('gallery: every entry renders clean at every viewport and theme', async ({ page }) => {
  const { captures, failures } = await walkGallery(page, {
    ...
    checks: async (p, label, vp) => {
      const found: string[] = [];
      const board = p.locator('[role="grid"][aria-label="tafl board"]').first();
      if ((await board.count()) > 0) {
        const cells = await p.locator('[role="gridcell"]').count();
        if (cells !== 121) found.push(`${label}: board has ${cells} cells, expected 121`);
