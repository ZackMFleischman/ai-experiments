// tafl's lobby doc→summary mapping over @parlor/web's listener hooks. The
// parlor hook owns the query/cache mechanics; the seat naming and card
// fields are tafl's. Seats: 'attackers' = 0 (moves first), 'defenders' = 1.
import type { DocumentData } from 'firebase/firestore';
import { useMyGames } from '@parlor/web/lobby';
import { deserializeTafl } from '@tafl/engine';
...
export function toSummary(id: string, raw: DocumentData, uid: string): LobbyGameSummary {
  ...
  return {
    ...
    board: deserializeTafl(data.state).board,
    moveCount: data.moveCount ?? 0,
  };
}

export function useTaflGames(uid: string): { games: LobbyGameSummary[]; loading: boolean } {
