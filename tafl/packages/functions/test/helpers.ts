...
const FUNCTIONS = 'http://127.0.0.1:5001/demo-tafl/us-central1';
const FIRESTORE = 'http://127.0.0.1:8080/v1/projects/demo-tafl/databases/(default)/documents';
...
/** Admin-bypass partial update (emulator only): only the given fields change
 * (PATCH without an updateMask would replace the whole document). Lets tests
 * rig a mid-game position under a game created through the real callables. */
export async function adminUpdateDoc(path: string, data: Record<string, unknown>): Promise<void> {
  ...
  if (!res.ok) throw new Error(`adminUpdateDoc ${path} failed: ${await res.text()}`);
}
