// @tafl/functions — server-authoritative game API: every callable is a
// @parlor/server shell shaped by tafl's config. createGame / joinGame /
// cancelGame / challengeUser / respondChallenge / rematch / resign
// (createGameCallables), submitMove (createSubmitMove over the engine
// `advance`), and the hourly forfeit sweep (createForfeitHandlers — tafl's
// toMove IS a seat key, so the generic sweep needs nothing game-side).
// Hnefatafl's only draw here is engine-derived (threefold repetition), so the
...
import { taflServerConfig, taflSubmitConfig } from './config';
...
export const { createGame, joinGame, cancelGame, challengeUser, respondChallenge, rematch, resign } =
  createGameCallables(taflServerConfig);

export const submitMove = createSubmitMove(taflSubmitConfig);

const handlers = createForfeitHandlers(taflServerConfig);
