...
describe('illegal moves', () => {
  ...
  it('rejects moving the opponent\'s piece', () => {
    expect(() => applyTafl(s, { from: at(2, 3), to: at(2, 1) })).toThrow(IllegalMoveError);
  });

  it('rejects moving from an empty square', () => {
    expect(() => applyTafl(s, { from: at(4, 4), to: at(4, 5) })).toThrow(IllegalMoveError);
  });

  it('rejects an occupied destination', () => {
    expect(() => applyTafl(s, { from: at(0, 3), to: at(1, 3) })).toThrow(IllegalMoveError);
  });
  ...
  it('rejects jumping over a piece', () => {
    expect(() => applyTafl(s, { from: at(0, 3), to: at(2, 3) })).toThrow(IllegalMoveError);
  });
  ...
  it('rejects a non-king landing on a corner or the throne', () => {
    ...
    expect(() => applyTafl(open, { from: at(3, 1), to: 24 })).toThrow(IllegalMoveError);
  });
  ...
  it('rejects any move once the game is over', () => {
    ...
    expect(() => applyTafl(won, { from: at(6, 2), to: at(6, 3) })).toThrow(IllegalMoveError);
  });
});
