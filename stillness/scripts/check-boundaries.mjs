...
// Stillness is a zero-backend app (no Firebase project at all), so this fails when:
...
//   (c) any stillness source imports another game's package.
...
// The other games' package scopes (stillness never imports a sibling game).
const OTHER_GAME_RE = /^@(hive|lex|checkers|tafl|sudoku|breakout)\//;
...
for (const file of sourceFiles(join(root, 'packages'))) {
  ...
  for (const spec of imports(text)) {
    if (FIREBASE_RE.test(spec)) {
      errors.push(`${file}: imports '${spec}' — Stillness is zero-backend; firebase must never enter the bundle`);
    }
    ...
    if (OTHER_GAME_RE.test(spec)) {
      errors.push(`${file}: imports '${spec}' — stillness never imports another game's packages`);
    }
  }
}
