...
### effect
- **bloom** — Glow: bright pixels bleed over the image (threshold → blur → add). `bloom(ctx, { input: src, level: 0.6, intensity: kickEnv })` _[bloom, glow, stateful, finish]_
- **blur** — Separable gaussian blur with a live pixel radius. `blur(ctx, { input: src, radius: 12 })` _[blur, soft, gaussian, stateful]_
...
- **crt** — CRT look: barrel curvature, scanlines, edge fringing in one step. `crt(ctx, { input: src, scan: 0.35, curve: 0.15 })` _[crt, retro, scanlines, finish, stateful]_
- **displace** — Warps the input's UVs by a displacer map (or built-in animated noise). `displace(ctx, { input: src, map: voronoi(ctx, {}), amount: 0.08 })` _[displace, warp, organic, stateful]_
- **echo** — Blends an N-frames-ago copy over the live frame (video echo/ghosting). `echo(ctx, { input: src, delay: 10, amount: 0.5 })` _[echo, delay, ghost, stateful]_
...
- **hsv** — Hue rotation, saturation and value adjust — the color half of grading. `hsv(ctx, { input: src, hue: lfo(ctx, { periodBeats: 32 }) })` _[hue, saturation, color, grade]_
- **invert** — Inverts colors, blendable 0..1 — the negative flash. `invert(ctx, { input: src, amount: ctx.input("kick") })` _[invert, negative, flash]_
...
- **key** — Keys a color (chroma) or dark pixels (luma) to transparency for layering. `key(ctx, { input: cam, mode: "luma", tolerance: 0.25 })` _[key, chroma, luma, alpha, layer]_
- **levels** — Gain, bias and gamma adjustment on an image. `levels(ctx, { input: src, gain: 1.2, gamma: 1.1 })` _[color, grade]_
- **mirror** — Folds the frame across an angled line — one-axis symmetry. `mirror(ctx, { input: src, angle: 0, offset: 0 })` _[mirror, symmetry, fold, stateful]_
...
- **posterize** — Quantizes colors to N steps per channel (poster/print-crush look). `posterize(ctx, { input: src, steps: 4 })` _[posterize, quantize, crush, retro]_
- **rgbSplit** — RGB channel split (chromatic aberration) with live amount + angle. `rgbSplit(ctx, { input: src, amount: kickEnv.map((k) => k * 0.02) })` _[rgb, aberration, split, glitch, stateful]_
- **threshold** — Keeps pixels above a luma level (soft edge), masks the rest to transparent. `threshold(ctx, { input: src, level: 0.6, softness: 0.1 })` _[threshold, mask, luma]_
- **tile** — Repeats the frame into an X×Y wall, optionally mirror-alternating for seamless edges. `tile(ctx, { input: src, countX: 3, countY: 3, mirrorTiles: 1 })` _[tile, repeat, grid, wall, stateful]_
...
- **vignette** — Darkens the frame's corners (radius/softness/amount) — the classic finish. `vignette(ctx, { input: src, amount: 0.7 })` _[vignette, finish, frame]_
...
- **camera-ghost** — Your camera, luma-keyed onto a palette wash, ghosted by frame echoes, poster-crushed — the kick flashes the negative. params: key.tolerance, key.softness, ghost.amount, ghost.delay, crush.steps, flash _[webcam, key, echo, audio-reactive, showcase]_
- **deck-mixer** — A two-deck video mixer: crossfader between clips, hue on a slow ride, kick-driven RGB split and frame echo for the drop. params: mix.fader, deckA.speed, deckB.speed, color.hue, color.saturation, drop.split, drop.echo, drop.delay _[video, mixer, deck, vj, audio-reactive, showcase]_
...
- **neon-bloom** — Neon rings detonate from the center on every kick and bloom over a slow radial palette wash. params: ring.punch, ring.release, ring.width, glow.level, glow.intensity, glow.radius, wash.level _[rings, bloom, palette, audio-reactive, showcase]_
...
- **rutt-etra** — A vertex terrain breathing with the bass, drawn as glowing scan points under a slow orbit — the Rutt-Etra look, three generations later. params: terrain.height, terrain.surge, terrain.scale, terrain.flow, points.size, cam.speed, cam.height _[3d, terrain, points, rutt-etra, audio-reactive, showcase]_
- **safe** — Slow-breathing radial gradient — the default PANIC safe scene (audio-independent). params: level, period _[panic, safe, minimal]_
- **spring-rave** — A ring of laser beams on spring physics — every kick bounces and rings instead of easing, hats lift the glow, trails smear the orbit. params: bounce.stiffness, bounce.damping, bounce.amount, beams.shimmer, cam.speed, trail _[3d, beams, spring, physics, audio-reactive, showcase]_
- **type-strobe** — Title cards step line-by-line on the kick — each hit re-rolls the placement, the bass gate strobes the backdrop. params: type.size, type.throw, strobe.amount, strobe.threshold _[text, strobe, type, audio-reactive, showcase]_
...
- **warp-room** — A palette wash warped by living voronoi cells, folded through a mirror and tiled into a breathing wall — bass pushes the warp. params: warp.amount, warp.surge, warp.cells, warp.drift, fold.angle, wall.countX, wall.countY _[displace, voronoi, symmetry, audio-reactive, showcase]_
