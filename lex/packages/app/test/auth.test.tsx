// ported from hive/packages/app/test/auth.test.tsx (adapted)
// T4.2: auth context + route guards. The screens are mode-aware through
// AuthContext; these tests inject fake auth values — the real firebase-backed
// provider (@parlor/web AppSyncProviders) is exercised by the emulator e2e.
...
describe('auth route guards (full mode)', () => {
  ...
  it('guards /new and /join behind sign-in', () => {
    renderAt('/new', fullAuth());
    expect(screen.getByRole('button', { name: /sign in with google/i })).toBeTruthy();
  });
...
describe('hot-seat mode (default context)', () => {
  it('keeps the landing Play flow with no auth', () => {
    ...
    expect(screen.getByRole('link', { name: /play hot-seat/i })).toBeTruthy();
...
describe('landing hero (T4.2)', () => {
  it('spells the wordmark from real tile components on a board vignette', () => {
    render(
      <MemoryRouter initialEntries={['/']}>
        <AppProviders>
          <AppRoutes />
    ...
    for (const letter of ['L', 'E', 'X']) {
      expect(hero.querySelector(`[data-tile="${letter}"]`)).toBeTruthy();
    }
    // Real tiles carry the ruleset's point indexes (X = 8 in the classic set).
    expect(hero.querySelector('[data-tile="X"]')?.textContent).toContain('8');
  });
});
