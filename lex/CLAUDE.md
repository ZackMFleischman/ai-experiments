...
1. `DESIGN.md` — what & why. §2.2 (Ruleset), §5.4 (Dictionary), §6.2 (schema),
...
2. `IMPLEMENTATION.md` — the ordered task list. **§0 (build protocol) is
...
- `@lex/engine`, `@lex/dict`, and `@lex/platform/core` stay zero-dependency, pure
...
- `@lex/platform` never imports `@lex/engine`. Firebase imports only under
  `platform/web`, `platform/server`, `app/src/sync/`, `packages/functions/`.
