// ported from hive/e2e/playwright.mp.config.ts (adapted)
...
// Dev server (not preview): full mode connects to emulators via DEV; the
// emulator transport is forced to long polling (§8.3).
...
export default defineConfig({
  ...
  webServer: {
    command:
      'pnpm --filter @lex/app exec vite --mode multiplayer --port 5189 --strictPort --host 127.0.0.1',
