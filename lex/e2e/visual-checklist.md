...
- Join card lists board, dictionary + word count, time control and seat (FR-10)
  before the accept button, themed hero above. Waiting screen: invite link AND
  bare code with copy affordances; the challenge variant swaps copy + withdraw
  action; the board is never visible while the game is open.
