...
//   (a) firebase escapes its confinement: inside checkers, firebase imports are
...
//   (b) any checkers source imports another game's package (@hive/*).
...
for (const file of sourceFiles(join(root, 'packages'))) {
  ...
  for (const spec of imports(text)) {
    ...
    if (spec.startsWith('@hive/')) {
      errors.push(`${file}: imports '${spec}' — checkers never imports another game's packages`);
    }
  }
}
